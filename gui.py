...
def stereo_rect(stereo_path, baseline=0.1, cameraMatrix=None):
    if cameraMatrix is None:
        cameraMatrix = np.array(
            [[1000, 0, 1920 / 2], [0, 1000, 1080 / 2], [0, 0, 1]]
    ...
    imgL_before_rec, _ = draw_epilines(
        imgL, imgR, left_before_lines, pts_left, pts_right
    )
    ...
    imgR_before_rec, _ = draw_epilines(
        imgR, imgL, right_before_lines, pts_right, pts_left
    )
...
def stereo_geometry_estimation(stereo_path, baseline=0.1, cameraMatrix=None):
    ...
    return {"Essential Matrix": E, "Rotation Matrix": R, "Translation Vector": T}
...
class NotebookGUI:
    ...
    def create_cam_calib_tab(self):
        ...
        ttk.Label(tab1, text="Chesseboard Calibration Patterns Folder Path:").grid(
        ...
        browse_btn = ttk.Button(
            tab1, text="Browse", command=self.cam_calib_browse_folder
        )
        ...
        self.cam_calib_output_text = scrolledtext.ScrolledText(
            tab1, width=70, height=20, wrap=tk.WORD
        )
    ...
    def run_cam_calib(self):
        ...
        for result in self.cam_calib_results:
            self.cam_calib_output_text.insert(
                tk.END, str(result[0]) + "\n" + str(result[1]) + "\n"
            )

    def cam_calib_browse_folder(self):
        folder = filedialog.askdirectory(
            title="Select Folder with Chesseboard Calibration Patterns"
        )
        if folder:
    ...
    def create_stereo_rect_tab(self):
        ...
        ttk.Label(controls_frame, text="Stereo Image Pair Folder Path:").grid(
            row=0, column=0, padx=5, pady=5, sticky="e"
        )
        ...
        ttk.Label(controls_frame, text="Baseline (millimeters):").grid(
        ...
        for i, (title, label) in enumerate(self.stereo_img_labels.items()):
            ttk.Label(tab2, text=title).grid(
                row=3 + 2 * (i // 2), column=i % 2, pady=(0, 2), sticky="n"
            )
    ...
    def run_stereo_rect(self):
        ...
        if not folder_path:
            for label in self.stereo_img_labels.values():
                label.config(image="")
                tk.Label(label.master, text="Please provide a folder path.").grid(
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Input", f"Invalid baseline value: {e}. Using default (0.1)."
            )
            baseline = 0.1

        self.stereo_rect_results = stereo_rect(
            folder_path,
            baseline=baseline,
            cameraMatrix=(
                self.cam_calib_results[0][1] if self.cam_calib_results else None
            ),
        )
        ...
        if isinstance(self.stereo_rect_results, str):  # Error case
            for label in self.stereo_img_labels.values():
                label.config(image="")
                tk.Label(label.master, text=self.stereo_rect_results).grid(
                    row=7, column=0, columnspan=2
                )
            return
        ...
        for title, img in self.stereo_rect_results.items():
            ...
            self.stereo_img_labels[title].image = (
                img_pil  # Keep a reference to avoid garbage collection
            )
    ...
    def create_feat_detect_match_tab(self):
        ...
        ttk.Label(controls_frame, text="Image Pair Folder Path:").grid(
            row=0, column=0, padx=5, pady=5, sticky="e"
        )
        ...
        ttk.Label(controls_frame, text="SIFT Contrast Threshold:").grid(
            row=1, column=0, padx=5, pady=5, sticky="e"
        )
        ...
        run_btn = ttk.Button(
            controls_frame, text="Run", command=self.run_feat_detect_match
        )
        run_btn.grid(row=2, column=1, pady=10)
        ...
        for i, title in enumerate(
            ["Left Image with Keypoints", "Right Image with Keypoints"]
        ):
        ...
        ttk.Label(tab3, text="Matched Images Before Lowe's Ratio").grid(
            row=5, column=0, columnspan=2, pady=(0, 2), sticky="n"
        )
        self.feat_img_labels["Matched Images Before Lowe's Ratio"].grid(
            row=6, column=0, columnspan=2, padx=5, pady=5
        )

        ttk.Label(tab3, text="Matched Images After Lowe's Ratio").grid(
            row=7, column=0, columnspan=2, pady=(0, 2), sticky="n"
        )
        self.feat_img_labels["Matched Images After Lowe's Ratio"].grid(
            row=8, column=0, columnspan=2, padx=5, pady=5
        )

        # Output text for debug/info
        self.feat_detect_match_output_text = scrolledtext.ScrolledText(
            tab3, width=70, height=5, wrap=tk.WORD
        )
        self.feat_detect_match_output_text.grid(
    ...
    def run_feat_detect_match(self):
        ...
        if not folder_path:
            ...
            self.feat_detect_match_output_text.insert(
                tk.END, "Please provide a folder path.\n"
            )
            return
        ...
        try:
            ...
            if (
                contrast_threshold < 0 or contrast_threshold > 0.1
            ):  # Reasonable range for contrast threshold
                raise ValueError("Contrast threshold must be between 0 and 0.1.")
        except ValueError as e:
            messagebox.showerror(
                "Invalid Input",
                f"Invalid contrast threshold: {e}. Using default (0.04).",
            )
        ...
        self.feat_detect_match_results = feat_detect_match(
            folder_path, contrast_threshold=contrast_threshold
        )
        ...
        if isinstance(self.feat_detect_match_results, str):  # Error case
            ...
            self.feat_detect_match_output_text.delete(1.0, tk.END)
            self.feat_detect_match_output_text.insert(
                tk.END, self.feat_detect_match_results + "\n"
            )
            return
        ...
        for title, img in self.feat_detect_match_results.items():
            ...
            self.feat_img_labels[title].image = (
                img_pil  # Keep a reference to avoid garbage collection
            )
    ...
    def create_stereo_geometry_estimation_tab(self):
        ...
        # Baseline input
        ttk.Label(controls_frame, text="Baseline (meters):").grid(
            row=1, column=0, padx=5, pady=5, sticky="e"
        )
        ...
        # Run button
        run_btn = ttk.Button(
            controls_frame, text="Run", command=self.run_stereo_geometry_estimation
        )
        run_btn.grid(row=2, column=1, pady=10)

        # Output text for displaying results
        self.stereo_geometry_output_text = scrolledtext.ScrolledText(
            tab4, width=70, height=20, wrap=tk.WORD
        )
        self.stereo_geometry_output_text.grid(
    ...
    def run_stereo_geometry_estimation(self):
        ...
        if not folder_path:
            ...
            self.stereo_geometry_output_text.insert(
                tk.END, "Please provide a folder path.\n"
            )
            return
        ...
        if isinstance(self.stereo_geometry_results, str):  # Error case
            self.stereo_geometry_output_text.insert(
                tk.END, self.stereo_geometry_results + "\n"
            )
            return

        self.stereo_geometry_output_text.insert(
            tk.END, "Stereo Geometry Estimation Results:\n"
        )
