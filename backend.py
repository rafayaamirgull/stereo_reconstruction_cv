...
def _find_files(folder_path, prefixes=('cam1', 'cam2'), extensions=('.jpg', '.png')):
    ...
    for ext in extensions:
        for prefix in prefixes:
            pattern_lower = os.path.join(
                folder_path, f"{prefix}_*{ext.lower()}")
            pattern_upper = os.path.join(
                folder_path, f"{prefix}_*{ext.upper()}")
            found_files.extend(glob.glob(pattern_lower))
            found_files.extend(glob.glob(pattern_upper))
            pattern_title = os.path.join(
                folder_path, f"{prefix}_*{ext.title()}")
    ...
    for f in set(found_files):
        ...
        base_prefix_matches = any(basename.startswith(
            p.lower() + '_') for p in prefixes)
        base_ext_matches = any(basename.endswith(e.lower())
                               for e in extensions)
...
def _parse_timestamp(filename):
    ...
    match = re.match(
        r"^(?:cam1|cam2)_([^\.]+)\.(?:jpg|png)$", basename, re.IGNORECASE)
    if match:
        return match.group(1)
    return None


def _find_stereo_pair(folder_path):
    ...
    cam2_timestamps = {_parse_timestamp(
        f): f for f in cam2_files if _parse_timestamp(f)}
    ...
    for cam1_path in cam1_files:
        ...
        if ts1 and ts1 in cam2_timestamps:
            ...
            print(
                f"Found pair: L='{os.path.basename(cam1_path)}', R='{os.path.basename(cam2_path)}'")
...
def stereo_calib(calib_path):
    print("--- Stereo Calibration ---")
    checkerboard_size = (9, 7)
    criteria_subpix = (cv2.TERM_CRITERIA_EPS +
    ...
    all_files = _find_files(calib_path, prefixes=[
                            'cam1', 'cam2'], extensions=['.jpg', '.png'])
    ...
    for timestamp, files in files_by_timestamp.items():
        if len(files) == 2:
            ...
            if cam1_path and cam2_path:
                valid_pairs.append(
                    {'ts': timestamp, 'L': cam1_path, 'R': cam2_path})
            else:
                print(
                    f"Warn: TS {timestamp}, 2 files, but not cam1/cam2: {files}")
        elif len(files) > 2:
    ...
    for i, pair_info in enumerate(valid_pairs):
        ...
        print(
            f"Processing {i+1}/{len(valid_pairs)} (TS: {pair_info['ts']}): L='{os.path.basename(img_l_path)}', R='{os.path.basename(img_r_path)}'")
        sys.stdout.flush()
        ...
        ret_l, corners_l = cv2.findChessboardCorners(
            gray_l, checkerboard_size, None)
        ret_r, corners_r = cv2.findChessboardCorners(
            gray_r, checkerboard_size, None)
    
        if ret_l and ret_r:
            print(f"  Found corners.")
            valid_pairs_processed += 1
            corners_l_subpix = cv2.cornerSubPix(
                gray_l, corners_l, (11, 11), (-1, -1), criteria_subpix)
            corners_r_subpix = cv2.cornerSubPix(
                gray_r, corners_r, (11, 11), (-1, -1), criteria_subpix)
    ...
    ret_l, M1_init, d1_init, _, _ = cv2.calibrateCamera(
        objpoints, imgpoints_l, img_shape, None, None)
    ret_r, M2_init, d2_init, _, _ = cv2.calibrateCamera(
    ...
    sys.stdout.flush()
    camera_model = {"M1": M1, "d1": d1, "M2": M2, "d2": d2, "R": R, "T": T,
...
def draw_epilines(img1, img2, lines, pts1, pts2):
    ...
    for r_line, pt1_coords, pt2_coords in zip(lines, pts1, pts2):
        ...
        try:
            ...
            ret_clip, p1_clip, p2_clip = cv2.clipLine(
                (0, 0, c, r), (x0, y0), (x1, y1))
            if ret_clip:
                img1_color = cv2.line(img1_color, tuple(
                    p1_clip), tuple(p2_clip), color, 1)
            img1_color = cv2.circle(
                img1_color, tuple(pt1_coords), 5, color, -1)
            img2_color = cv2.circle(
                img2_color, tuple(pt2_coords), 5, color, -1)
...
def stereo_rect(stereo_path, M1, d1, M2, d2, R, T, F, image_size):
    print("--- Stereo Rectification (Using Calibrated Parameters) ---")
    left_image_path, right_image_path = _find_stereo_pair(stereo_path)
    ...
    if input_image_size != tuple(image_size):
        print(
            f"Warn: Input size {input_image_size} != calib size {tuple(image_size)}.")
            ...
            if len(matches) > 0:
                ...
                if F is not None:
                    ...
                    imgL_before_rec_draw, _ = draw_epilines(
                        imgL_before_rec_draw, imgR_color.copy(), lines1, pts1_vis, pts2_vis)
                    lines2 = cv2.computeCorrespondEpilines(pts1_vis, 1, F)
                    imgR_before_rec_draw, _ = draw_epilines(
                        imgR_before_rec_draw, imgL_color.copy(), lines2, pts2_vis, pts1_vis)
                else:
    ...
    try:
        R1, R2, P1, P2, Q, _, _ = cv2.stereoRectify(
            M1, d1, M2, d2, tuple(image_size), R, T, alpha=0.0)
    ...
    try:  # Epiline drawing after
        ...
        if des1_rect is not None and des2_rect is not None and len(des1_rect) > 1 and len(des2_rect) > 1:
            ...
            matches_rect = sorted(bf_rect.match(
                des1_rect, des2_rect), key=lambda x: x.distance)[:50]
            if len(matches_rect) > 0:
                ...
                for m in matches_rect:
                    ...
                    y = pt1_rect[1]
                    color = tuple(np.random.randint(0, 255, 3).tolist())
                    cv2.line(imgL_after_rec_draw, (0, y),
                             (w_rect, y), color, 1)
                    cv2.line(imgR_after_rec_draw, (0, y),
                             (w_rect, y), color, 1)
    ...
    return {"Original Left": imgL_before_rec_draw, "Original Right": imgR_before_rec_draw, "Drawn Rectified Left": imgL_after_rec_draw, "Drawn Rectified Right": imgR_after_rec_draw, "Rectified Left": imgL_rect_gray, "Rectified Right": imgR_rect_gray, "Rectified Color Left": imgL_rect_color, "Rectified Color Right": imgR_rect_color, "disp2depth map": Q}

# --- Feature Detection and Matching Function --- 
def feat_detect_match(stereo_path, cameraMatrix1=None):
    ...
    if cameraMatrix1 is None:
        ...
        f = max(w_img, h_img)
        K = np.array([[f, 0, w_img/2], [0, f, h_img/2],
    ...
    E_feat, mask_e = cv2.findEssentialMat(
    ...
    imgL_kp = cv2.drawKeypoints(
        imgL_color, kp_l, None, color=(0, 255, 0), flags=0)
    imgR_kp = cv2.drawKeypoints(
        imgR_color, kp_r, None, color=(0, 255, 0), flags=0)
    ...
    return {"Left Image with Keypoints": imgL_kp, "Right Image with Keypoints": imgR_kp, "Matched Images Before Lowe's Ratio": img_raw, "Matched Images After Lowe's Ratio": img_good, "Left Aligned Keypoints": pts1.reshape(-1, 2), "Right Aligned Keypoints": pts2.reshape(-1, 2), "Fundamental Matrix Feature": F_feat, "Essential Matrix Feature": E_feat, "Rotation Matrix Feature": R_feat, "Translation Vector Feature": t_feat, "Left Color Image": imgL_color, "Num Keypoints Left": nkp_l, "Num Keypoints Right": nkp_r, "Num Raw Matches": n_raw, "Num Good Matches": n_good}

# --- Stereo Geometry Estimation Function ---
def stereo_geometry_estimation(stereo_path, cameraMatrix1=None):
    ...
    if F is None:
        ...
        print(
            f"F RANSAC inliers: {np.sum(mask_f) if mask_f is not None else 'N/A'}")
    ...
    if n_inl_e < 5:
        ...
        try:
            ...
            if R_p is None or T_v_p is None:
                ...
                R, T_v = R_p, T_v_p
                print(
                    f"recoverPose inliers: {np.sum(mask_p) if mask_p is not None else 'N/A'}")
    ...
    return {"Fundamental Matrix": F, "Essential Matrix": E, "Determinant E": det_E, "Singular Values E": svd_E, "Non Zero SVD E": nz_svd_E, "Rotation Matrix": R, "Translation Vector": T_v}


# --- Triangulation Function ---
def triangulation_and_3D_reconstruction(pts1_matched, pts2_matched, cameraMatrix1, rotationMatrix, translationVector, imgL_color):
    ...
    # --- Filtering ---
    # Keep points with positive Z in the *first* camera's view
    # Keep a very generous max depth filter
    z_positive_mask = points_3d[:, 2] > 0
    # Extremely large depth limit, effectively only filters infinite points if any slip through
    filter_z_max = 10000.0
    max_depth_mask = points_3d[:, 2] < filter_z_max

    # Combine essential filters
...
def disparity_calculation(imgL_rect, imgR_rect, guide_image=None):
    ...
    stereo_l = cv2.StereoSGBM_create(minDisparity=min_disp, numDisparities=num_disp, blockSize=block_sz, P1=P1, P2=P2,
                                     disp12MaxDiff=disp12Max, uniquenessRatio=unique, speckleWindowSize=speckleWin, speckleRange=speckleRng, mode=mode)
    lambda_w = 8000.0
    sigma_c = 1.5
    ...
    try:
        if guide.shape[:2] != disp_l_raw.shape[:2]:
            guide_res = cv2.resize(
                guide, (disp_l_raw.shape[1], disp_l_raw.shape[0]))
        else:
            guide_res = guide
        filtered_disp_s = wls.filter(
            disp_l_raw, guide_res, disparity_map_right=disp_r_raw)
    ...
    # Define mask based on SGBM's minDisparity (scaled) and finiteness
    valid_disp_mask = (filtered_disp_s > (min_disp * 16)
                       ) & np.isfinite(filtered_disp_f)  # boolean mask
    ...
    if np.any(valid_disp_mask):
        ...
        print(
            f"Normalization range (float, valid): min={min_v:.2f}, max={max_v:.2f}")
        if max_v > min_v:
            # --- Convert boolean mask to uint8 for normalize ---
            norm_image = cv2.normalize(src=filtered_disp_f, dst=None, alpha=0, beta=255,
    ...
    sys.stdout.flush()
    return {"Disparity Map": filtered_vis_gray, "Disparity Map Color": filtered_vis_color, "Raw Disparity": filtered_disp_f}

# --- Visualize Point Cloud from Disparity (More Robust Filtering) ---
...
def visualize_point_cloud_disparity(raw_disparity_map, Q, colors):
    ...
    try:
        ...
        min_disp_val = 0.01  # Slightly lower threshold, still > 0
        max_depth_val = 10000.0  # Significantly increased depth limit

        print(
            f"Filtering points: Disp > {min_disp_val}, Depth > 0, Depth < {max_depth_val}, IsFinite")

        # 1. Filter based on minimum disparity
        ...
        # 2. Filter based on finite 3D coordinates (essential)
        ...
        # # 3. Filter based on positive depth (essential)
        # positive_z_mask = points_3D[:, :, 2] > 0
        # num_after_pos_z = np.sum(disp_mask & finite_mask & positive_z_mask)
        # print(f"  Points after positive Z filter: {num_after_pos_z}")

        # # 4. Filter based on maximum depth (relaxed)
        # depth_mask = points_3D[:, :, 2] < max_depth_val
        # num_after_depth = np.sum(
        #     disp_mask & finite_mask & positive_z_mask & depth_mask)
        # print(f"  Points after max depth filter: {num_after_depth}")

        # Combine all masks
        final_mask = disp_mask & finite_mask  # & positive_z_mask & depth_mask

        num_valid_final = np.sum(final_mask)
        if num_valid_final == 0:
            print(
                "Warn: No valid points remaining after all filters. Cannot generate point cloud.")
            print(
                "      Possible causes: Poor disparity map, invalid Q matrix, extreme scene scale.")
            sys.stdout.flush()
            return "Warning: No valid points after filtering."  # Return warning, not error
        ...
        if valid_rgb.shape[0] == valid_pts.shape[0]:
            ...
            print(
                f"Warn: Point/color mismatch ({valid_pts.shape[0]} vs {valid_rgb.shape[0]}).")
    ...
    # Return None on success, error message otherwise
    return warning_msg
